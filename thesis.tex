...
L'obiettivo di questo elaborato è studiare il fenomeno del domain shift, in particolare analizzare come le performance di un modello degradano nei tre casi precedentemente descritti: sim-to-sim (nel caso di ambienti in uno stesso simulatore), real-to-real e sim-to-real. Viene inoltre proposta un'architettura applicabile nel contesto della \textit{fog robotics}, utilizzata per condurre ulteriori esperimenti e considerazioni sul domain shift.

\subsection{Task del modello}
\label{sec:task_del_modello}
...
\subsection{Architettura per fog robotics}
\label{sec:architettura_modello_encoder_decoder}

Oltre ad analizzare le performance di un modello nei tre casi precedentemente descritti, in questo elaborato viene proposta e testata un'architettura alternativa per cercare di ridurre il domain shift in un contesto di fog robotics. La fog robitics~\cite{10.1145/2757384.2757397} è un'estensione del concetto di cloud robotics~\cite{qian2009cloud}, e punta a ridurre latenza e consumo di banda, distribuendo il carico computazionale tra il cloud e i nodi fog più vicini ai robot. Uno schema che illustra il concetto di fog robotics è mostrato nella \hyperref[fig:fog_robotics]{Figura \ref{fig:fog_robotics}}.

\begin{figure}[t]
	\centering
	\includegraphics[width=0.8\textwidth, clip]{images/fog-robotics}
	\caption{Schema di un sistema fog robotics.}
...
Nel seguente capitolo vengono analizzati gli strumenti utilizzati, motivandone le scelte. Inoltre, vengono descritti i processi di creazione e addestramento dei modelli, presentando frammenti di codice esemplificativi per illustrarne l'implementazione.
