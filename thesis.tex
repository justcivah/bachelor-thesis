...
\subsubsection{Sim-to-sim}
\label{sec:sim_to_sim}

Come primo caso si è analizzato il domain shift tra ambienti simulati, in particolare è stato testato il modello all'interno degli stessi ambienti utilizzati per l'addestramento. Nella pratica sono state prese immagini e dati di segmentazione semantica da 7 ambienti diversi, nei quali si sono campionati dati attraverso 30 diverse run. Di questi dati raccolti, 35000 immagini sono state utilizzate per l'addestramento, mentre le altre 5000 sono state usate per la validazione. Il modello è stato addestrato sulla macchina PC1 per 60 epoche, con una dimensione di batch pari a 8 e risoluzione delle immagini di 512 pixel. Il processo di training è durato circa 16 ore e 20 minuti e le metriche finali sono riportate nella \hyperref[fig:sim-to-sim-1]{Figura \ref{fig:sim-to-sim-1}}.
...
Il secondo scenario studiato riguarda
