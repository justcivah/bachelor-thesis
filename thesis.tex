...
Come discusso nel \hyperref[cap:introduzione]{Capitolo \ref{cap:introduzione}}, lo scopo di questo elaborato è analizzare il fenomeno del domain shift in tre scenari differenti: sim-to-sim, real-to-real e sim-to-real. L’obiettivo finale è quantificare il calo prestazionale di un modello di robotic vision in ciascun caso, identificando i motivi di tale comportamento. Viene inoltre effettuata l'analisi del domain shift in un contesto reale distribuito, proponendo un’architettura che combina un segmentatore semantico e un encoder-decoder. Questo design consente di sfruttare i vantaggi dei sistemi distribuiti, come l’elaborazione parallela e la riduzione del carico computazionale sui dispositivi edge.

Nel seguente capitolo vengono analizzate le tecnologie e gli strumenti utilizzati, motivandone le scelte e descrivendo il loro impiego all'interno di questo lavoro. Inoltre, vengono descritti i processi di creazione e addestramento dei modelli, presentando frammenti di codice esemplificativi che illustrano l'implementazione.
...
Mentre l'analisi delle performance nei tre scenari principali (sim-to-sim, real-to-real e sim-to-real) richiede l'utilizzo di un singolo modello di riferimento, lo studio del fenomeno in un'architettura distribuita obbliga a riprogettare l'intera pipeline di lavoro.
