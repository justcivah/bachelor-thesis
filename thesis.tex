...
L'obiettivo di questo elaborato è studiare il fenomeno del domain shift, analizzando in particolare come le performance di un modello degradano nei tre casi precedentemente descritti: sim-to-sim (nel caso di ambienti in uno stesso simulatore), real-to-real e sim-to-real. Inoltre viene approfondito il problema del domain shift in un contesto reale e sempre più diffuso, quello della \textit{fog robotics}, attraverso lo sviluppo di un modello dedicato.
...
Oltre ad analizzare le performance nei tre casi precedentemente descritti, in questo elaborato viene proposta e testata un'architettura alternativa per studiare il problema del domain shift in un sistema distribuito di fog robotics. La fog robitics~\cite{10.1145/2757384.2757397} è un'estensione del concetto di cloud robotics~\cite{qian2009cloud}, e punta a ridurre latenza e consumo di banda, distribuendo il carico computazionale tra il cloud e i nodi fog più vicini ai robot. Uno schema generico che illustra il concetto di fog robotics è mostrato nella \hyperref[fig:fog_robotics]{Figura \ref{fig:fog_robotics}}.
