...
Riassunendo quanto visto, in questo elaborato si è analizzato il fenomeno del domain shift in vari scenari, e si è realizzata un'architettura per ridurre il domain shift in un sistema di fog robotics.

Si è scoperto che un segmentatore validato negli stessi ambienti utilizzati durante il training performa in maniera eccellente. Questo effetto è più marcato in ambienti simulati, mentre è meno presetne in quelli reali. Questo è principalmente dovuto al fatto che le scene del simulatore non presentano un livello di eterogeneità paragonabili a quelle del simulatore. Inoltre, le annotazioni semantiche dei dati simulati sono perfette, mentre quelle provenienti dal dataset ScanNet presentano diverse imperfezioni.

Se invece si effettua testing su ambienti diversi da quelli di addestramento, il modello addestrato con dati simulati ne risente maggiormente, mentre quello addestrato con dati reali generalizza meglio su dati non visti. Questa differenza di comportamento è causata dal fatto che i dati reali sono stati campionati da un numero di scene maggiore rispetto a quelli simulati. Un altro fattore importante e da tenere in considerazione è l'alto livello di eterogeneità tra i diversi ambienti.

Per quanto riguarda il domain shift sim-to-real, i due domini sono troppo differenti per permettere a questo specifico modello di generalizzare correttamente. Una soluzione potrebbe essere utilizzare un simulatore più fotorealistico, che permetta di avere texture di alta qualità, un sistema di illuminazione dimanico, e magari strumenti per effettuare automaticamente domain randomization. È inoltre essenziale riuscire a campionare dati da un gran numero di ambienti simulati; come visto nell'\hyperref[sec:esperimento_6]{Esperimento 6} un gran numero di ambienti di training permette di poter generalizzare su scene mai viste prima.

Analizzando invece l'architettura fog robotics, si può notare che nel caso real-to-real il modello UNet-YOLO permette di colmare parte del gap effettuando fine-tuning con immagini provenienti dal dominio di destinazione. Si passa da una mAP50 di 0.411 utilizzando solo YOLO, ad una mAP50 di 0.502 nel caso del modello distribuito. Inoltre si è 

Nel caso invece sim-to-real, quest'architettura coma 
